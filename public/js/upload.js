    ...
    if (ev.target.tagName === 'INPUT' && ev.target.type === 'file') {
        ...
        for (const file of ev.target.files) {
            ...
            if (!(new RegExp('^[a-zA-Z0-9_\-]+\\.[a-z0-9]+$', 'g')).test(file.name)) {
                errors.push("Filename %FILENAME% contains disallowed characters. Only a-z, A-Z, 0-9, _ and - are allowed, and the file extension must exist, be lowercase, and contain only a-z and 0-9.".replace('%FILENAME%', file.name));
                continue;
            }
